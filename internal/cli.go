...
func (s *SSLHandshake) SetUsage() {
	flag.Usage = func() {
		...
		fmt.Printf("  %s -c 3 tuladhar.github.com:443\n", s.Metadata.Name)
		fmt.Printf("  %s -t 500 imap.gmail.com:993\n", s.Metadata.Name)
	}
}

func (s *SSLHandshake) ParseFlags() {
	flag.Int64Var(&s.Config.Timeout, "t", s.Config.Timeout, "SSL handshake timeout in milliseconds.")
	flag.Int64Var(&s.Config.StopCount, "c", s.Config.StopCount, "Stop after <count> SSL handshakes.")
